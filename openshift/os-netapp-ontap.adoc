...
有關 NetApp ONTAP 儲存系統的詳細資訊，請造訪 link:https://www.netapp.com/data-management/ontap-data-management-software/["NetApp ONTAP網站"^]。
...
如需 ONTAP 的詳細資訊，請參閱 link:https://docs.netapp.com/ontap-9/index.jsp["ONTAP 9 文檔中心"^]。
...
Amazon FSx ONTAP 在 AWS 雲端中提供完全代管的共享儲存設備，並具備 ONTAP 廣受歡迎的資料存取和管理功能。如需 Amazon FSx for NetApp ONTAP 的詳細資訊，請參閱 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/what-is-fsx-ontap.html["Amazon FSx for NetApp ONTAP 文件"^]。
...
Azure NetApp Files 是一項 Azure 原生、第一方、企業級且高效能的檔案儲存設備服務。它以服務形式提供 Volume，您可以為其建立 NetApp 帳戶、容量池和 Volume。您還可以選擇服務與效能等級，並管理資料保護。您可以使用在內部部署環境中熟悉且信賴的相同協定和工具，來建立和管理高效能、高可用度的且易擴充的檔案共享。如需有關 Azure NetApp Files 的詳細資訊，請參閱 link:https://learn.microsoft.com/en-us/azure/azure-netapp-files/["Azure NetApp Files 文件"^]。
...
Google Cloud NetApp Volumes 是一項完全代管的雲端型資料儲存設備服務，提供進階資料管理員功能和易擴充的高效能。它可讓您將以檔案為基礎的應用程式移轉至 Google Cloud。它內建支援網路檔案系統 （NFSv3 和 NFSv4.1） 以及伺服器訊息區塊 （SMB） 通訊協定，因此您無需重新設計應用程式架構，即可繼續為應用程式取得持續儲存。如需 Google Cloud NetApp Volumes 的詳細資訊，請參閱 link:https://cloud.google.com/netapp/volumes/docs/discover/overview["Google Cloud NetApp Volumes 說明文件"^]。
