...
此解決方案利用部署在 VMware vSphere 中的虛擬應用裝置。F5 Big-IP 虛擬應用裝置的網路可根據您的網路環境設定為雙臂或三臂組態。本文件中的部署以雙臂組態為基礎。有關設定虛擬應用裝置以搭配 Anthos 使用的更多詳細資訊，請參閱 link:https://cloud.google.com/solutions/partners/installing-f5-big-ip-adc-for-gke-on-prem["F5 BIG-IP ADC GKE On-Prem 安裝指南"^]。
...
. 從 link:https://downloads.f5.com/esd/serveDownload.jsp?path=/big-ip/big-ip_v15.x/15.0.1/english/virtual-edition/&sw=BIG-IP&pro=big-ip_v15.x&ver=15.0.1&container=Virtual-Edition&file=BIGIP-15.0.1-0.0.11.ALL-vmware.ova["F5 下載入口網站"^] 下載虛擬應用裝置 Open Virtual Appliance （OVA） 檔案。
+
