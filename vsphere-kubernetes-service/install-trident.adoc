...
NOTE: 此時 Trident Pod 可能無法建立。如果您描述該 ReplicaSet 物件，可能會看到違反 PodSecurity 的錯誤，如下所示。出現此錯誤的原因是 `trident`安裝期間建立的命名空間強制執行了 Kubernetes 受限的 Pod 安全標準 （PSS）。Trident Operator 需要提升的系統權限才能管理主機儲存設備，因此無法在受限設定檔的嚴格約束下運作。若要解決此問題，請為 `trident`命名空間授予特權設定檔，以便 ReplicaSet 控制器能夠成功啟動 Pod。
...
----
將內建的 Kubernetes Pod Security Admission 標籤套用至 `trident`命名空間，使其不受受限設定檔約束：
